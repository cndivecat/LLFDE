...
import cv2
def vis_img(image,img_name):
    image = image.squeeze(0)
    mean=torch.tensor([123.675, 116.28, 103.53], device=image.device)
    std=torch.tensor([58.395, 57.12, 57.375], device = image.device)
    while len(mean.shape) < len(image.shape):
        mean = mean.unsqueeze(-1)
    while len(std.shape) < len(image.shape):
        std = std.unsqueeze(-1)
    #逆归一化
    ...
    cv2.imwrite("visualize/"+ img_name +".png", image)


def vis_fea(x,fea_name):
    x = x.squeeze(0)
    x = torch.mean(x, dim=0)
    x = F.softmax(x)
    plt.figure()
    plt.imshow(x.cpu().detach().numpy(), cmap='viridis') 
    plt.axis('off')
    plt.savefig("visualize/"+ fea_name +".png")
...
class ResNetFuseFrequencyDecouplingHFDESE(BaseBackbone):
    """ResNet backbone.
    ...
    def __init__(self,
        ...
        # Protect mutable default arguments
        norm_cfg = copy.deepcopy(norm_cfg)
        super(ResNetFuseFrequencyDecouplingHFDESE, self).__init__(init_cfg)
    ...
    def init_weights(self):
        ...
        super(ResNetFuseFrequencyDecouplingHFDESE, self).init_weights()
...
@MODELS.register_module()
class ResNetV1dFuseFrequencyDecouplingHFDESE(ResNetFuseFrequencyDecouplingHFDESE):
...
class ExtendLayer(nn.Module):
    def __init__(self):
        ...
        # 全局特征的获取
        self.glob_conv1 = ConvModule(in_channels=32,out_channels=16,kernel_size=5,padding=2)
        ...
        self.loaclconv = ConvModule(in_channels=48,out_channels=48,kernel_size=1,act_cfg=None)
        self.Lendconv = ConvModule(in_channels=48,out_channels=3,kernel_size=1,act_cfg=None)
    ...
    def forward(self,x):
        ...
        fea_glob = self.glob_conv3(self.glob_conv2(self.glob_conv1(fea_lay1_3)))
        ...
        fea_U4 = self.deconv4(fea_lay4_3)
        fea_U3 = self.deconv3(torch.concat([fea_U4,fea_lay4_2],1))
        fea_L1 = self.Lendconv(self.loaclconv(torch.concat([fea_U3,fea_lay4_1],1)))
...
class FrequencyModule(nn.Module):
    ...
    def window_partition(self, x, window_size):
        ...
        """
        B, H, W, C = x.shape
        x = x.view(B, H // window_size, window_size, W // window_size, window_size, C)#（B, H/8, 8, W/8, 8, C）
        windows = x.permute(0, 1, 3, 2, 4, 5).contiguous().view(-1, window_size, window_size, C)    #(B*HW/64, 8, 8, C)
        windows = windows.view(-1, window_size * window_size, C)      #(B*HW/64, 64, C)
        return windows
    ...
    def window_reverse(self, windows, window_size, H, W):
        ...
        x = windows.view(B, H // window_size, W // window_size, window_size, window_size, -1)       #(B*HW/64, 8, 8, C)
    ...
    def forward(self,x):
        ...
        feaHF_enhanced = feaHF_enhanced.permute(0,3,1,2).contiguous()
        vis_fea(feaHF_enhanced,'ehf')
        # feaLF_enhanced = self.dem(feaLF)
        # fuseFea = self.feamix(feaLF_enhanced,feaHF_enhanced)
        return feaHF_enhanced
