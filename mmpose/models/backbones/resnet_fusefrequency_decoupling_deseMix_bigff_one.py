...
class ResNetFuseFrequencyDecouplingDESEMixBigffOne(BaseBackbone):
    """ResNet backbone.
    ...
    def __init__(self,
        ...
        # Protect mutable default arguments
        norm_cfg = copy.deepcopy(norm_cfg)
        super(ResNetFuseFrequencyDecouplingDESEMixBigffOne, self).__init__(init_cfg)
    ...
    def init_weights(self):
        ...
        super(ResNetFuseFrequencyDecouplingDESEMixBigffOne, self).init_weights()
...
@MODELS.register_module()
class ResNetV1dFuseFrequencyDecouplingDESEMixBigffOne(ResNetFuseFrequencyDecouplingDESEMixBigffOne):
...
class ExtendLayer(nn.Module):
    def __init__(self):
        ...
        # 全局特征的获取
        self.glob_conv1 = ConvModule(in_channels=8,out_channels=16,kernel_size=5,padding=2)
        ...
        self.loaclconv = ConvModule(in_channels=16,out_channels=16,kernel_size=1,act_cfg=None)
        self.Lendconv = ConvModule(in_channels=16,out_channels=3,kernel_size=1,act_cfg=None)
        self.Gendconv = ConvModule(in_channels=16,out_channels=3,kernel_size=1,act_cfg=None)    #先插值再卷积



    def forward(self,x):
        fea_lay1_1 = self.conv1_1_2(self.conv1_1_1(x))   #(256,192,8)
        ...
        fea_glob = self.glob_conv3(self.glob_conv2(self.glob_conv1(fea_lay1_1)))   #(256,192,16)

        fea_lay2_1 = self.conv2_1(fea_lay1_1)   #(256,192,16)
        # fea_lay2_2 = self.conv2_2(fea_lay1_2)
        # fea_lay2_3 = self.conv2_3(fea_lay1_3)
        # fea_lay2_4 = self.conv2_4(fea_lay1_4)

        fea_lay4_1 = self.LHFrequency1(fea_lay2_1)   #(256,192,16)
        ...
        fea_L1 = self.Lendconv(self.loaclconv(fea_lay4_1))   #(256,192,3)
        # fea_G = self.Gendconv(F.interpolate(input=fea_glob,size=(256,192),mode='bilinear'))
        fea_G = self.Gendconv(fea_glob)   #(256,192,3)

        fea = fea_L1 + fea_G   #(256,192,3)
